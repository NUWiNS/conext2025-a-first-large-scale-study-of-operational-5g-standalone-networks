...
tmobile_nsa_app_dir = "../raw_data/small_study_chic_bos/boston/tmobile_nsa_boston_drive"
tmobile_sa_app_dir = "../raw_data/small_study_chic_bos/boston/tmobile_sa_boston_drive"

tmobile_nsa_xcal_file = "../raw_data/small_study_chic_bos/boston/tmobile_nsa_boston_drive/TMOBILE_NSA_100MS.xlsx"
tmobile_sa_xcal_file = "../raw_data/small_study_chic_bos/boston/tmobile_sa_boston_drive/TMOBILE_SA_100MS.xlsx"
...
if 1:
    ...
    if not os.path.exists("../pkls/data_boston_may_2025/tmobile_nsa_xcal_df.pkl"):
        ...
        fh = open("../pkls/data_boston_may_2025/tmobile_nsa_xcal_df.pkl", "wb")
        pkl.dump(tmobile_nsa_xcal_df, fh)
        fh.close()
    else:
        fh = open("../pkls/data_boston_may_2025/tmobile_nsa_xcal_df.pkl", "rb")
...
if 1:
    ...
    if not os.path.exists("../pkls/data_boston_may_2025/tmobile_sa_xcal_df.pkl"):
        ...
        fh = open("../pkls/data_boston_may_2025/tmobile_sa_xcal_df.pkl", "wb")
        pkl.dump(tmobile_sa_xcal_df, fh)
        fh.close()
    else:
        fh = open("../pkls/data_boston_may_2025/tmobile_sa_xcal_df.pkl", "rb")
...
if 1:
    ...
    if 1:
        ...
        plt.tight_layout()
        plt.savefig('../plots/yearwise/fig_7b_tmobile_nsa_sa_dl_xput_boston.pdf')
        plt.close()
...
if 1:
    ...
    if 1:
        ...
        plt.tight_layout()
        plt.savefig('../plots/yearwise/fig_7d_tmobile_nsa_sa_ul_xput_boston.pdf')
        plt.close()
...
if 1:
    if 1:
        merged_nsa_ping_df = pd.concat(ping_df_list_nsa)
        merged_nsa_ping_df = merged_nsa_ping_df.sort_values("Timestamp").reset_index(drop=True)
        merged_nsa_ping_df = merged_nsa_ping_df[['Timestamp', 'Lat', 'Lon', 'Event Technology', '5G KPI PCell RF Serving PCI', '5G KPI PCell RF NR-ARFCN', '5G KPI PCell RF Band', '5G KPI PCell RF BandWidth', '5G KPI Total Info Layer2 MAC UL Throughput [Mbps]', '5G KPI Total Info Layer2 MAC DL Throughput [Mbps]']]
        merged_sa_ping_df = pd.concat(ping_df_list_sa)
        merged_sa_ping_df = merged_sa_ping_df.sort_values("Timestamp").reset_index(drop=True)
        merged_sa_ping_df = merged_sa_ping_df[['Timestamp', 'Lat', 'Lon', 'Event Technology', '5G KPI PCell RF Serving PCI', '5G KPI PCell RF NR-ARFCN', '5G KPI PCell RF Band', '5G KPI PCell RF BandWidth', '5G KPI Total Info Layer2 MAC UL Throughput [Mbps]', '5G KPI Total Info Layer2 MAC DL Throughput [Mbps]']]
        ...
        temp = merged_df[['5G KPI PCell RF Serving PCI_x', '5G KPI PCell RF Serving PCI_y', '5G KPI PCell RF NR-ARFCN_x', '5G KPI PCell RF NR-ARFCN_y']].dropna()
        nsa_arfcn_list = list(temp['5G KPI PCell RF NR-ARFCN_x'])
        sa_arfcn_list =  list(temp['5G KPI PCell RF NR-ARFCN_y'])
        nsa_pci_list = list(temp['5G KPI PCell RF Serving PCI_x'])
        sa_pci_list =  list(temp['5G KPI PCell RF Serving PCI_y'])

        for nsa_pci, sa_pci, nsa_arfcn, sa_arfcn in zip(nsa_pci_list, sa_pci_list, nsa_arfcn_list, sa_arfcn_list):
            if nsa_pci == sa_pci:
                if nsa_arfcn == sa_arfcn:
                    same_pci_same_arfcn+=1
                else:
...
if 1:
    ...
    plt.tight_layout()
    plt.savefig('../plots/yearwise/fig_4b_tmobile_nsa_sa_bw_PCI_ARFCN_boston_flattened.pdf')
    plt.close()